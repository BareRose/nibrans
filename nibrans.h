...
NBRADEF size_t nibransEncodeChunk(struct nibrans*, unsigned char*, size_t, const unsigned char*, size_t);
    //uses the given nibrans context (must have been initialized) to encode binary data from
    //given input buffer (last 2 arguments) to given output buffer (2nd and 3rd argument)
    //encodes up to 4096 bytes of data (a chunk), effectively clamping input size to 4096
    //returns number of bytes written to output, or 0 on failure (e.g. buffer too small)
NBRADEF size_t nibransDecodeChunk(struct nibrans*, unsigned char*, size_t, const unsigned char*, size_t);
    //uses the given nibrans context (must have been initialized) to decode binary data from
    //given input buffer (last 2 arguments) to given output buffer (2nd and 3rd argument)
    //decodes up to 4096 bytes of data (a chunk), effectively clamping output size to 4096
    //returns number of bytes read from input, or 0 on failure (e.g. buffer too small)
...
#define NBRA_RATE_BITS 6 //number of rate bits for adaption shift
...
//macros
#define NBRA_MIXIN(I, J) J + ((I < J) ? NBRA_PROB_SIZE - 16 : 0)
...
NBRADEF size_t nibransEncode (struct nibrans* nbra, unsigned char* out, size_t out_size, const unsigned char* in, size_t in_size) {
    ...
    while (1) {
        ...
        if (!(ret = nibransEncodeChunk(nbra, out_cur, out_rem, in_cur, in_rem))) return 0;
        //advance out cursor
        out_cur = &out_cur[ret]; out_rem -= ret;
        //advance in cursor or exit
        if (in_rem > NBRA_CHNK_SIZE) {
            in_rem -= NBRA_CHNK_SIZE;
            in_cur = &in_cur[NBRA_CHNK_SIZE];
        } else break;
    }
    //return
    return out_cur - out;
...
NBRADEF size_t nibransDecode (struct nibrans* nbra, unsigned char* out, size_t out_size, const unsigned char* in, size_t in_size) {
    ...
    while (out_size) {
        //return 0 if any one chunk returns 0, indicating failure
        if (!(ret = nibransDecodeChunk(nbra, out_cur, out_rem, in_cur, in_rem))) return 0;
        //advance in cursor
        in_cur = &in_cur[ret]; in_rem -= ret;
        //advance out cursor or exit
        if (out_rem > NBRA_CHNK_SIZE) {
            out_rem -= NBRA_CHNK_SIZE;
            out_cur = &out_cur[NBRA_CHNK_SIZE];
        } else break;
    }
    //return
    return in_cur - in;
}
NBRADEF size_t nibransEncodeChunk (struct nibrans* nbra, unsigned char* out, size_t out_size, const unsigned char* in, size_t in_size) {
    //two nibbles in a byte, so we need two ranges per input byte
    struct nbra_range rng_buff[NBRA_CHNK_SIZE*2];
    //clamp in_size to NBRA_CHNK_SIZE
    if (in_size > NBRA_CHNK_SIZE)
        in_size = NBRA_CHNK_SIZE;
    //process data forwards for modeling
    for (size_t i = 0; i < in_size; i++) {
        //grab nibbles for this byte
        unsigned char n1 = in[i] >> 4;
        unsigned char n2 = in[i] & 0x0F;
        //insert ranges of nibbles into buffer
        rng_buff[i*2] = nbraModRange(nbra->cdf1, n1);
        rng_buff[i*2+1] = nbraModRange(nbra->cdf2, n2);
        nbraModUpdate(nbra->cdf1, n1);
        nbraModUpdate(nbra->cdf2, n2);
    }
...
NBRADEF size_t nibransDecodeChunk (struct nibrans* nbra, unsigned char* out, size_t out_size, const unsigned char* in, size_t in_size) {
    //clamp out_size to NBRA_CHNK_SIZE
    if (out_size > NBRA_CHNK_SIZE)
        out_size = NBRA_CHNK_SIZE;
    ...
    for (size_t i = 0; i < out_size; i++) {
        ...
        unsigned char n1 = nbraModSymbol(nbra->cdf1, nbraDecGet(&cod1));
        unsigned char n2 = nbraModSymbol(nbra->cdf2, nbraDecGet(&cod2));
        struct nbra_range rng1 = nbraModRange(nbra->cdf1, n1);
        struct nbra_range rng2 = nbraModRange(nbra->cdf2, n2);
        nbraModUpdate(nbra->cdf1, n1);
        nbraModUpdate(nbra->cdf2, n2);
...
static unsigned char nbraModSymbol (const uint16_t cdf[17], uint16_t prb) {
    //returns the symbol the range of which matches the given prb in given model
        ...
        //load cdf values with proper offsets
        __m128i cdf1 = _mm_loadu_si128((__m128i*)&cdf[1]);
        __m128i cdf9 = _mm_loadu_si128((__m128i*)&cdf[9]);
        //return symbol based on trailing zeroes of packed (comp < cdf1) and (comp < cdf9) using SSE2
        return NIBRANS_CTZ(_mm_movemask_epi8(_mm_packs_epi16(_mm_cmplt_epi16(comp, cdf1), _mm_cmplt_epi16(comp, cdf9))));
    #else
...
static void nbraModUpdate (uint16_t cdf[17], unsigned char c) {
    ...
    static const uint16_t mixin[16][16] = NBRA_MIXIN_INIT;
    #ifndef NIBRANS_NO_SSE2
        //load values into aligned __m128i variables
        __m128i mix0 = _mm_loadu_si128((__m128i*)&mixin[c][0]);
        __m128i mix8 = _mm_loadu_si128((__m128i*)&mixin[c][8]);
        __m128i cdf0 = _mm_loadu_si128((__m128i*)&cdf[0]);
        __m128i cdf8 = _mm_loadu_si128((__m128i*)&cdf[8]);
        //do rhs calculations: (mix - cdf) >> NBRA_RATE_BITS
        __m128i rhs0 = _mm_srai_epi16(_mm_sub_epi16(mix0, cdf0), NBRA_RATE_BITS);
        __m128i rhs8 = _mm_srai_epi16(_mm_sub_epi16(mix8, cdf8), NBRA_RATE_BITS);
        //add rhs to cdf and store results in model memory
        _mm_storeu_si128((__m128i*)&cdf[0], _mm_add_epi16(cdf0, rhs0));
        _mm_storeu_si128((__m128i*)&cdf[8], _mm_add_epi16(cdf8, rhs8));
    #else
        //no SSE variant (shifted up to avoid subtraction underflow)
        for (int i = 1; i < 16; i++)
            cdf[i] = (((uint32_t)cdf[i] << NBRA_RATE_BITS) + mixin[c][i] - cdf[i]) >> NBRA_RATE_BITS;
    #endif
}
